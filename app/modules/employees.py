...
class EmployeesApp(QWidget):
    ...
    def __init__(self, user_data=None, parent=None): # Ebeveyn kuralı gereği parent=None ekledik
        ...
        if hasattr(self, "btn_excel_yukle"):
            self.btn_excel_yukle.clicked.connect(self.import_excel)
    ...
    def import_excel(self):
        """Excel (.xlsx) dosyasından personel verilerini içe aktarır."""
        try:
            import openpyxl
        except Exception as e:
        ...
        try:
            ...
            # DB'deki son PERxxxx kodunu baz al
            next_code_num = 0
            conn = None
            try:
                conn = self.db.connect()
                if conn is not None:
                    ...
                    cursor.execute(
                        ...
                        WHERE personel_kodu IS NOT NULL
                          AND personel_kodu LIKE 'PER%'
                        ORDER BY CAST(SUBSTR(personel_kodu, 4) AS INTEGER) DESC
                        LIMIT 1
                        """
                    )
                    last_row = cursor.fetchone()
                    if last_row and last_row[0]:
                        m = re.search(r"(\d+)$", str(last_row[0]).strip())
                        if m:
                            next_code_num = int(m.group(1))
            except Exception:
                next_code_num = 0
            finally:
                try:
            ...
            def generate_personel_kodu() -> str:
                nonlocal next_code_num
                while True:
                    next_code_num += 1
                    candidate = f"PER{next_code_num:04d}"
                    if not self.db.get_employee_details(candidate):
                        return candidate

            # Header satırı (1. satır)
            raw_headers = []
            for cell in ws[1]:
            ...
            header_idx = {norm(h): i for i, h in enumerate(raw_headers)}

            def get_cell(row_vals, *names, default=""):
                for n in names:
                    i = header_idx.get(norm(n))
            ...
            for row in ws.iter_rows(min_row=2, values_only=True):
                ...
                personel_kodu = get_cell(row_vals, "personel_kodu", "kodu", "kod", "personel kodu", "personel_kod")
                if not personel_kodu:
                    personel_kodu = generate_personel_kodu()

                tckn = get_cell(row_vals, "tckn", "tc", "tc kimlik", "tc kimlik no", "tc kimlik no.")
                ad_soyad = get_cell(row_vals, "ad_soyad", "adı soyadı", "adi soyadi", "ad soyad")
                personel_turu = get_cell(row_vals, "personel_turu", "tür", "tur")
                gorevi = get_cell(row_vals, "gorevi", "görevi", "gorev", "görev")
                gsm = get_cell(row_vals, "gsm", "telefon", "tel")
                email = get_cell(row_vals, "email", "e-posta", "e posta", "eposta")
                kan_grubu = get_cell(row_vals, "kan_grubu", "kan grubu", "kan")
                durum_txt = get_cell(row_vals, "durumu", "durum", "aktif")
                ...
                exists = self.db.get_employee_details(personel_kodu)
                ok = self.db.save_employee(data, is_update=bool(exists))
                if ok:
                    if exists:
                        updated += 1
                    else:
            ...
            QMessageBox.information(
                self,
                "Başarılı",
                f"Excel içe aktarım tamamlandı.\n\nYeni: {imported}\nGüncellenen: {updated}\nAtlanan: {skipped}"
            )

        except Exception as e:
            QMessageBox.critical(self, "Hata", f"Excel içe aktarım hatası:\n{str(e)}")
